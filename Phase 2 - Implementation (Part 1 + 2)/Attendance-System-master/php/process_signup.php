...
/************ SEARCH ***********/
$con = connectTo();
$exists = $con->query("select * from `appedemia`.`teacher` where email = '$email'");
if($exists && $con->affected_rows) {
  $con->close();
  respond("error","exists");
} 
/*********** INSERT ************/
$addTeacher = $con->query("insert into `appedemia`.`teacher` (`name`, `email`, `phone`, `password`) values ('$name', '$email', '$phone', '$pass')");
